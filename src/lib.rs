/*
 * @Author: your name
 * @Date: 2021-03-31 15:14:30
 * @LastEditTime: 2021-03-31 15:24:52
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \rsa_sign\src\lib.rs
 */
pub mod der_encoded;
pub mod verify_sign;
pub mod hash;
pub mod send_sign;
#[cfg(test)]
mod tests {
    use crate::send_sign::send_sign;
    use crate::verify_sign::verify_sign;
    #[test]
    fn it_works() {
        assert_eq!(2 + 2, 4);
    }
    #[test]
    fn test_sign() {
        let _ = "";
let file_content = r#"
-----BEGIN RSA PUBLIC KEY-----

RwIDAQAB
-----END RSA PUBLIC KEY-----
"#;
   
let private_file_content = r#"
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"#;
    let sign_string = send_sign(private_file_content, &"a=2&b=3&c=4");
    assert_eq!(sign_string.is_empty(), false);
    let is_ok = verify_sign(file_content, &sign_string, "a=2&b=3&c=4".to_string());
    assert_eq!(is_ok, true);
    }
}
